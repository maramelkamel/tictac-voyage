...
// ── Detect Duffel offer-expiry errors ─────────────────────────────
...
const searchFlights = async (req, res) => {
  try {
    ...
    const result = await duffelService.searchFlights({
      slices, passengers, cabin_class:cabin_class??'economy', max_connections,
    });
...
const bookFlight = async (req, res) => {
  try {
    ...
    const normalizedPassengers = passengers.map(p => ({
      ...p, phone_number: toE164(p.phone_number || ''),
    }));

    // Fetch fresh offer (ensures it hasn't expired)
    let offer;
    try {
    ...
    try {
      order = await duffelService.bookFlight({
        offer_id, passengers:normalizedPassengers, payments:paymentPayload,
      });
    } catch (err) {
      if (isOfferExpiredError(err)) {
        return res.status(410).json({
    ...
    // Fired for BOTH payment methods — online gets "confirmed", agency gets "pending"
    const emailStatus   = isOnline ? 'confirmed' : 'pending';
...
// ── Admin: update status ──────────────────────────────────────────
// Only main admin can cancel. Email sent on every status change.
const updateReservationStatus = async (req, res) => {
  try {
    ...
    // ── 🔔 Send status update email on every meaningful change ────
...
const setPriceOverride = async (req, res) => {
  try {
    ...
    if (isNaN(price) || price <= 0)
      return res.status(400).json({ success:false, message:'overridden_price doit être positif.' });
