...
const createReservation = async (data) => {
  ...
  const { rows } = await pool.query(`
    INSERT INTO public.voyage_reservations
      (voyage_id, first_name, last_name, email, phone, chambre_type, number_of_persons, total_price, payment_method, payment_status, status, notes)
...
const deleteReservation = async (id) => {
  const { rows } = await pool.query(
    'DELETE FROM public.voyage_reservations WHERE id=$1 RETURNING id',
    [id]
  );
  return rows[0] || null;
};
