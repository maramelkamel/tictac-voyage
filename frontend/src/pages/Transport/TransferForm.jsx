...
export function TransferForm() {
  ...
  const getDepartureDisplay = () => {
    if (formData.departureType === 'airport') {
      ...
      return airport ? `${airport.name} (${airport.city})` : 'Sélectionner un aéroport';
    } else if (formData.departureType === 'hotel') {
      return formData.departureHotel ? `${formData.departureHotel}, ${formData.departureCity}` : 'Saisir le nom de l\'hôtel';
    }
    return formData.departureAddress || 'Saisir l\'adresse';
  };
  ...
  const getArrivalDisplay = () => {
    if (formData.arrivalType === 'airport') {
      ...
      return formData.arrivalHotel ? `${formData.arrivalHotel}, ${formData.arrivalCity}` : 'Saisir le nom de l\'hôtel';
    }
    return formData.arrivalAddress || 'Saisir l\'adresse';
  };
          ...
          {['airport', 'hotel', 'address'].map(type => (
            <label key={type} className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-[#1E293B] capitalize">
                {type === 'airport' ? 'Aéroport' : type === 'hotel' ? 'Hôtel' : 'Adresse'}
              </span>
            </label>
          ))}
        </div>
