...
const StatusBadge = ({ s }) => {
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '3px 9px',
      borderRadius: 999, fontSize: 11, fontWeight: 600, background: m.bg, color: m.color, whiteSpace: 'nowrap' }}>
...
const PaymentCell = ({ method, status }) => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
    ...
    {status === 'paid' && (
      <span style={{ fontSize: 10, fontWeight: 700, color: '#065f46', paddingLeft: 2 }}>✓ Payé</span>
    )}
  </div>
);
...
const ResDetail = ({ res, onClose, onStatusChange, isMain }) => {
  if (!res) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
      gap: 16, height: '100%', padding: 32, textAlign: 'center' }}>
      <svg viewBox="0 0 24 24" fill="none" stroke="var(--g300)" strokeWidth="1"
        style={{ width: 56, height: 56 }}>
        <path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 9.81 19.79 19.79 0 01.22 1.18 2 2 0 012.22 0h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.91 7.09a16 16 0 006 6l.66-.66a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 14.92z"/>
      </svg>
  ...
  const pax = (() => {
    try { return typeof res.passengers === 'string' ? JSON.parse(res.passengers) : res.passengers; }
    catch { return []; }
  })();

  const Section = ({ title, children }) => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      <p style={{ fontSize: 10, fontWeight: 700, color: 'var(--g400)', textTransform: 'uppercase',
        letterSpacing: '.1em', paddingBottom: 8, borderBottom: '1px solid var(--g100)' }}>
        {title}
      </p>
      {children}
    </div>
  ...
  const Item = ({ label, value, full }) => !value ? null : (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 3, gridColumn: full ? '1 / -1' : undefined }}>
      <span style={{ fontSize: 10, fontWeight: 600, color: 'var(--g400)', textTransform: 'uppercase', letterSpacing: '.05em' }}>
        {label}
      </span>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      {/* Header */}
      <div style={{ padding: '18px 20px', borderBottom: '1px solid var(--g100)',
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <p style={{ fontWeight: 800, fontSize: 16, color: 'var(--g900)' }}>
            {res.origin_iata || '?'} → {res.destination_iata || '?'}
          </p>
          <p style={{ fontSize: 12, color: 'var(--g400)', marginTop: 3 }}>
            #{res.id} · {res.airline_name || 'Compagnie inconnue'}
          </p>
        </div>
        <button onClick={onClose}
          style={{ width: 32, height: 32, borderRadius: 8, border: '1.5px solid var(--g200)',
            background: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          <svg viewBox="0 0 24 24" fill="none" stroke="var(--g500)" strokeWidth="2" style={{ width: 14, height: 14 }}>
            <path d="M18 6L6 18M6 6l12 12" />
          </svg>
        </button>
      </div>

      {/* Agency pending banner */}
      {isAgencyPending && (
        <div style={{ margin: '12px 20px 0', padding: '12px 16px', background: '#fff7ed',
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <Section title="Itinéraire">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            ...
            <Item label="Départ"       value={fDT(res.departing_at)} full />
            <Item label="Arrivée"      value={fDT(res.arriving_at)} full />
        ...
        {pax.length > 0 && (
          <Section title={`Passagers (${pax.length})`}>
            {pax.map((p, i) => (
              <div key={i} style={{ padding: '8px 12px', background: 'var(--g50)', borderRadius: 8,
                fontSize: 13, color: 'var(--g700)', display: 'flex', justifyContent: 'space-between' }}>
                <span>{p.given_name} {p.family_name}</span>
                <span style={{ fontSize: 11, color: 'var(--g400)', textTransform: 'capitalize' }}>{p.type}</span>
              </div>
            ))}
          </Section>
        )}
        ...
        <Section title="Paiement">
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center', marginTop: 4 }}>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 12px',
              ...
              background: res.payment_method === 'online' ? '#eff6ff' : '#fff7ed',
              color:      res.payment_method === 'online' ? '#1d4ed8' : '#c2410c' }}>
            ...
            {res.payment_status === 'paid' && (
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '4px 10px',
                borderRadius: 8, background: '#d1fae5', color: '#065f46', fontSize: 12, fontWeight: 700 }}>
                ✓ Payé
              </span>
            )}
          </div>
        ...
        <Section title="Changer le statut">
          {isAgencyPending && (
            <div style={{ padding: '10px 14px', background: '#fff7ed', borderRadius: 8,
              fontSize: 12, color: '#92400e', marginBottom: 8, lineHeight: 1.5 }}>
              ⚠️ Cliquez sur <strong>Confirmée</strong> après réception du paiement.
            </div>
          )}
          {!isMain && (
            <div style={{ padding: '10px 14px', background: '#fef2f2', border: '1px solid #fecaca',
              borderRadius: 8, fontSize: 12, color: '#991b1b', marginBottom: 8 }}>
        ...
        {res.notes && (
          <Section title="Remarques">
            <p style={{ fontSize: 13, color: 'var(--g600)', lineHeight: 1.6,
              background: 'var(--g50)', padding: '10px 14px', borderRadius: 8 }}>
              {res.notes}
            </p>
          </Section>
        )}

        <div style={{ padding: '12px 14px', background: 'var(--g50)', borderRadius: 10,
          fontSize: 11, color: 'var(--g400)' }}>
          <p>Réservée le {fDT(res.created_at)}</p>
          {res.updated_at !== res.created_at && <p>Mise à jour le {fDT(res.updated_at)}</p>}
        </div>
      </div>
    </div>
  );
...
const FlightReservations = () => {
  ...
  const fetchReservations = async () => {
    try {
      ...
      notify('Impossible de charger les réservations', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const filtered = reservations.filter(r => {
    ...
    const pax = (() => {
      try { return typeof r.passengers === 'string' ? JSON.parse(r.passengers) : r.passengers; }
      catch { return []; }
    })();
    const paxStr = pax.map(p => `${p.given_name} ${p.family_name}`).join(' ').toLowerCase();

    return (
      (!search || paxStr.includes(q) || (r.origin_iata || '').toLowerCase().includes(q) ||
        (r.destination_iata || '').toLowerCase().includes(q) || (r.airline_name || '').toLowerCase().includes(q)) &&
  ...
  return (
    ...
    >
      ...
      <div className="al-stats">
        {[
          {
            label: 'Total réservations', value: stats.total, color: 'blue',
          ...
          {
            label: 'CA encaissé (TND)', value: stats.revenue.toLocaleString('fr-TN', { maximumFractionDigits: 0 }), color: 'purple',
      ...
      {stats.agencyPending > 0 && (
        <div style={{ margin: '0 32px 16px', padding: '14px 20px', background: '#fff7ed',
          border: '1px solid #fed7aa', borderRadius: 12, display: 'flex', alignItems: 'center', gap: 12 }}>
      ...
      {/* ── Main grid ────────────────────────────────────────────── */}
      <div style={{ display: 'grid', gridTemplateColumns: selectedRes ? '1fr 380px' : '1fr',
        gap: 0, margin: '0 0 32px', transition: 'grid-template-columns .3s' }}>

        {/* Table */}
        <div style={{ margin: '0 0 0 32px', background: '#fff', borderRadius: 16,
          border: '1px solid var(--g200)', boxShadow: 'var(--shadow-md)', overflow: 'hidden',
          ...
          <div className="al-toolbar" style={{ flexWrap: 'wrap', gap: 10 }}>
            <div className="al-search">
              ...
              <input type="text" placeholder="Passager, IATA, compagnie…"
            ...
            <div className="al-filter-tabs">
              {[
                { v: 'all',    l: 'Tous paiements' },
                { v: 'online', l: '💳 En ligne' },
                { v: 'agency', l: '🏪 Agence' },
              ].map(({ v, l }) => (
                <button key={v} className={`al-filter-tab ${filterPayment === v ? 'active' : ''}`}
            ...
            <div className="al-filter-tabs">
              {[
                ...
                <button key={v} className={`al-filter-tab ${filterStatus === v ? 'active' : ''}`}
          ...
          {loading ? (
            ...
            <div className="al-empty">
              ...
              <p className="al-empty__sub">Modifiez vos filtres.</p>
            </div>
          ) : (
                    ...
                    return (
                      <tr key={r.id} className="al-row"
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            ...
                            <div>
                              ...
                              <p style={{ fontSize: 11, color: 'var(--g400)', marginTop: 2 }}>
                                {CABIN_LABELS[r.cabin_class] || r.cabin_class}
                              </p>
                            </div>
                          </div>
                        </td>
                        ...
                        <td>
                          {firstPax ? (
                            <>
                              <p style={{ fontWeight: 600, fontSize: 13, color: 'var(--g800)' }}>
                                {firstPax.given_name} {firstPax.family_name}
                              </p>
                              {pax.length > 1 && (
                                <p style={{ fontSize: 11, color: 'var(--g400)', marginTop: 2 }}>
                                  + {pax.length - 1} autre{pax.length > 2 ? 's' : ''}
                                </p>
                              )}
                            </>
                          ) : (
                        ...
                        {/* Compagnie */}
                        <td>
                          <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--g700)' }}>
                            {r.airline_name || '—'}
                          </span>
                          {r.flight_number && (
                            <p style={{ fontSize: 11, color: 'var(--g400)', marginTop: 2 }}>{r.flight_number}</p>
                          )}
                        </td>
                        ...
                        {/* Date */}
                        <td>
                          <span style={{ fontSize: 12, color: 'var(--g500)' }}>{fDate(r.created_at)}</span>
                        </td>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                            {isAgencyPending && (
                              <button onClick={() => handleStatusChange(r.id, 'confirmed')}
                                style={{ padding: '4px 10px', borderRadius: 7, border: '1.5px solid #10b981',
                            ...
                            <select value={r.status}
                              ...
                              style={{ padding: '5px 8px', borderRadius: 7, border: '1.5px solid var(--g200)',
                                fontSize: 12, fontFamily: 'inherit', cursor: 'pointer', background: '#fff', outline: 'none' }}>
                              <option value="pending">En attente</option>
                              <option value="confirmed">Confirmer</option>
                              <option value="completed">Terminer</option>
                              <option value="cancelled" disabled={!isMain}
                                style={{ color: !isMain ? '#ccc' : undefined }}>
        ...
        {/* Detail panel */}
        {selectedRes && (
          <div style={{ margin: '0 32px 0 16px', background: '#fff', borderRadius: 16,
            border: '1px solid var(--g200)', boxShadow: 'var(--shadow-md)', overflow: 'hidden',
            display: 'flex', flexDirection: 'column' }}>
            <ResDetail
              res={selectedRes}
