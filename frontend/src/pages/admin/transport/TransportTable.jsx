...
const TransportTable = ({ transports, loading, onEdit, onDelete, isMain }) => {
  ...
  const SortArrow = ({ field }) => (
    <svg viewBox="0 0 24 24" fill="none" strokeWidth="2"
      style={{ width:13, height:13, marginLeft:4, verticalAlign:'middle', stroke: sortField===field?'var(--primary)':'var(--g300)' }}>
    ...
    <div style={{ display:'flex', minHeight:0 }}>
      ...
      {selectedTransport && (
        <div style={{ width:320, flexShrink:0, borderLeft:'1px solid var(--g200)', display:'flex', flexDirection:'column', background:'#fff', animation:'alModalIn .25s var(--ease)', overflowY:'auto' }}>
          ...
          {/* En-tête */}
          <div style={{ padding:'16px 18px 14px', borderBottom:'1px solid var(--g100)', display:'flex', alignItems:'center', justifyContent:'space-between', position:'sticky', top:0, background:'#fff', zIndex:2 }}>
            <p style={{ fontSize:11, fontWeight:700, color:'var(--g400)', textTransform:'uppercase', letterSpacing:'.1em' }}>Détails véhicule</p>
          ...
          <div style={{ width:'100%', height:160, background:'var(--g100)', flexShrink:0, position:'relative', overflow:'hidden' }}>
            {selectedTransport.image_url ? (
              <img src={selectedTransport.image_url} alt={selectedTransport.transport_name} style={{ width:'100%', height:'100%', objectFit:'cover', display:'block' }}/>
            ) : (
              <div style={{ width:'100%', height:'100%', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:8 }}>
                <svg viewBox="0 0 24 24" fill="none" stroke="var(--g300)" strokeWidth="1.2" style={{ width:40, height:40 }}>
                  <rect x="3" y="3" width="18" height="16" rx="2"/><path d="M3 9h18M8 9v10M13 9v10"/>
                </svg>
                <p style={{ fontSize:11, color:'var(--g400)' }}>Pas d'image</p>
              </div>
            )}
            <span className={`al-badge-pill ${selectedTransport.is_available?'b--green':'b--red'}`}
          ...
          <div style={{ padding:'14px 18px', borderTop:'1px solid var(--g100)', display:'flex', gap:8, position:'sticky', bottom:0, background:'#fff' }}>
            <button className="al-btn al-btn--primary" style={{ flex:1 }}
            ...
            <button
              ...
              title={isMain ? 'Supprimer ce véhicule' : 'Réservé à l\'administrateur principal'}
              style={{ opacity: isMain ? 1 : 0.4, cursor: isMain ? 'pointer' : 'not-allowed' }}
            >
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ width:14, height:14 }}>
