...
const ResDetail = ({ res, onClose, onStatusChange }) => {
  if (!res) return <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:16, height:'100%', padding:32, textAlign:'center' }}><svg viewBox="0 0 24 24" fill="none" stroke="var(--g300)" strokeWidth="1" style={{ width:56, height:56 }}><path d="M1 6v16l7-4 8 4 7-4V2l-7 4-8-4-7 4z"/><path d="M8 2v16M16 6v16"/></svg><p style={{ fontSize:14, fontWeight:600, color:'var(--g400)', lineHeight:1.6 }}>Cliquez sur une réservation<br/>pour voir les détails</p></div>;
  const isAgencyPending = res.payment_method==='agency' && res.status==='pending';
  const Section = ({ title, children }) => <div style={{ display:'flex', flexDirection:'column', gap:10 }}><p style={{ fontSize:10, fontWeight:700, color:'var(--g400)', textTransform:'uppercase', letterSpacing:'.1em', paddingBottom:8, borderBottom:'1px solid var(--g100)' }}>{title}</p>{children}</div>;
  const Item = ({ label, value, full }) => !value ? null : <div style={{ display:'flex', flexDirection:'column', gap:3, gridColumn:full?'1 / -1':undefined }}><span style={{ fontSize:10, fontWeight:600, color:'var(--g400)', textTransform:'uppercase', letterSpacing:'.05em' }}>{label}</span><span style={{ fontSize:13, fontWeight:500, color:'var(--g700)' }}>{value}</span></div>;
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%', overflow:'hidden' }}>
      <div style={{ padding:'18px 20px', borderBottom:'1px solid var(--g100)', display:'flex', alignItems:'flex-start', gap:14, flexShrink:0 }}>
        ...
        <div style={{ flex:1, minWidth:0 }}><p style={{ fontWeight:800, fontSize:16, color:'var(--g900)' }}>{res.first_name} {res.last_name}</p><p style={{ fontSize:12, color:'var(--g400)', marginTop:3 }}>#{res.id} · {res.circuit_title||'Circuit supprimé'}</p></div>
        <button onClick={onClose} style={{ width:32, height:32, borderRadius:8, border:'1.5px solid var(--g200)', background:'#fff', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}><svg viewBox="0 0 24 24" fill="none" stroke="var(--g500)" strokeWidth="2" style={{ width:14, height:14 }}><path d="M18 6L6 18M6 6l12 12"/></svg></button>
      </div>
      {isAgencyPending && <div style={{ margin:'12px 20px 0', padding:'12px 16px', background:'#fff7ed', border:'1px solid #fed7aa', borderRadius:10, display:'flex', alignItems:'center', gap:10 }}><span style={{ fontSize:18 }}>🏪</span><div><p style={{ fontSize:12, fontWeight:700, color:'#c2410c' }}>Paiement à l'agence — En attente</p><p style={{ fontSize:11, color:'#92400e', marginTop:2 }}>Confirmez après réception du paiement.</p></div></div>}
      <div style={{ flex:1, overflowY:'auto', padding:'18px 20px', display:'flex', flexDirection:'column', gap:20 }}>
        <Section title="Contact client"><div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}><Item label="Prénom" value={res.first_name}/><Item label="Nom" value={res.last_name}/><Item label="Email" value={res.email} full/><Item label="Téléphone" value={res.phone}/></div></Section>
        <Section title="Circuit"><div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}><Item label="Circuit" value={res.circuit_title} full/><Item label="Chambre" value={res.chambre_type}/><Item label="Participants" value={`${res.number_of_persons} personne${res.number_of_persons>1?'s':''}`}/><Item label="Total" value={fPrice(res.total_price)}/></div></Section>
        <Section title="Paiement">
          <div style={{ display:'flex', gap:10, flexWrap:'wrap', alignItems:'center' }}>
            <span style={{ display:'inline-flex', alignItems:'center', gap:6, padding:'5px 12px', borderRadius:8, background:res.payment_method==='online'?'#eff6ff':'#fff7ed', color:res.payment_method==='online'?'#1d4ed8':'#c2410c', fontSize:12, fontWeight:600 }}>{res.payment_method==='online'?'💳 Paiement en ligne':"🏪 Paiement à l'agence"}</span>
        ...
        <Section title="Changer le statut">
          {isAgencyPending && <div style={{ padding:'10px 14px', background:'#fff7ed', borderRadius:8, fontSize:12, color:'#92400e', marginBottom:8, lineHeight:1.5 }}>⚠️ Cliquez sur <strong>Confirmée</strong> après paiement.</div>}
          <div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>
            {Object.entries(STATUS_MAP).map(([key, meta]) => (
              <button key={key} onClick={() => onStatusChange(res.id, key)} style={{ display:'flex', alignItems:'center', gap:7, padding:'7px 14px', borderRadius:8, border:`1.5px solid ${res.status===key?meta.color:'var(--g200)'}`, background:res.status===key?meta.bg:'#fff', fontSize:12, fontWeight:600, cursor:'pointer', fontFamily:'inherit', color:res.status===key?meta.color:'var(--g600)', transition:'all .2s' }}>
                <span style={{ width:7, height:7, borderRadius:'50%', background:meta.dot }}/>{meta.label}
              </button>
            ))}
          </div>
        </Section>
        {res.notes && <Section title="Remarques"><p style={{ fontSize:13, color:'var(--g600)', lineHeight:1.6, background:'var(--g50)', padding:'10px 14px', borderRadius:8 }}>{res.notes}</p></Section>}
        <div style={{ padding:'12px 14px', background:'var(--g50)', borderRadius:10, fontSize:11, color:'var(--g400)' }}><p>Réservée le {fDT(res.created_at)}</p></div>
      </div>
    </div>
  );
};
...
const CircuitReservations = () => {
  ...
  return (
    <AdminLayout title="Réservations Circuits" breadcrumb={[{ label:'Circuits' },{ label:'Réservations', active:true }]}
      ...
      {stats.agencyPending > 0 && (
        <div style={{ margin:'0 32px 16px', padding:'14px 20px', background:'#fff7ed', border:'1px solid #fed7aa', borderRadius:12, display:'flex', alignItems:'center', gap:12 }}>
          ...
          <div><p style={{ fontWeight:700, fontSize:14, color:'#c2410c' }}>{stats.agencyPending} réservation{stats.agencyPending>1?'s':''} en attente de paiement à l'agence</p><p style={{ fontSize:12, color:'#92400e', marginTop:2 }}>Confirmez après réception du paiement.</p></div>
        </div>
      )}
      ...
      <div style={{ display:'grid', gridTemplateColumns:selectedRes?'1fr 380px':'1fr', gap:0, margin:'0 0 32px', transition:'grid-template-columns .3s' }}>
        <div style={{ margin:'0 0 0 32px', background:'#fff', borderRadius:16, border:'1px solid var(--g200)', boxShadow:'var(--shadow-md)', overflow:'hidden', display:'flex', flexDirection:'column' }}>
          ...
          {loading ? <div className="al-loading"><div className="al-spinner-wrap"><div className="al-spinner"/></div><p style={{ fontSize:13, color:'var(--g400)' }}>Chargement...</p></div>
          : filtered.length === 0 ? <div className="al-empty"><div className="al-empty__icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.2"><circle cx="12" cy="12" r="10"/><path d="M12 8v4M12 16h.01"/></svg></div><p className="al-empty__title">Aucune réservation</p><p className="al-empty__sub">Modifiez vos filtres.</p></div>
          : (
                      ...
                      <tr key={r.id} className="al-row" style={{ cursor:'pointer', background:isSel?'#e0fbfc':isAgencyPending?'#fffbf5':undefined }} onClick={()=>setSelectedRes(isSel?null:r)}>
                        ...
                        <td onClick={e=>e.stopPropagation()}>
                          <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
                            {isAgencyPending && <button onClick={()=>handleStatusChange(r.id,'confirmed')} style={{ padding:'4px 10px', borderRadius:7, border:'1.5px solid #10b981', background:'#d1fae5', color:'#065f46', fontSize:11, fontWeight:700, cursor:'pointer', fontFamily:'inherit', whiteSpace:'nowrap' }}>✅ Confirmer</button>}
                            <select value={r.status} onChange={e=>handleStatusChange(r.id,e.target.value)} style={{ padding:'5px 8px', borderRadius:7, border:'1.5px solid var(--g200)', fontSize:12, fontFamily:'inherit', cursor:'pointer', background:'#fff', outline:'none' }}>
                              <option value="pending">En attente</option><option value="confirmed">Confirmer</option><option value="completed">Terminer</option><option value="cancelled">Annuler</option>
                            </select>
                          </div>
                        </td>
                      </tr>
        ...
        {selectedRes && (
          <div style={{ margin:'0 32px 0 16px', background:'#fff', borderRadius:16, border:'1px solid var(--g200)', boxShadow:'var(--shadow-md)', overflow:'hidden', display:'flex', flexDirection:'column' }}>
            <ResDetail res={selectedRes} onClose={()=>setSelectedRes(null)} onStatusChange={handleStatusChange}/>
          </div>
        )}
      </div>
    </AdminLayout>
