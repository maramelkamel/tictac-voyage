...
const CircuitPayment = () => {
    ...
    return (
      <div className="payment-page">
        ...
        <div style={{ textAlign: 'center', padding: '160px 24px' }}>
          ...
          <button className="payment-fallback__btn" onClick={() => navigate('/circuits')}>← Retour aux circuits</button>
        </div>
        <Footer />
      </div>
    );
  ...
  const saveReservation = async (paymentMethod) => {
    const payload = {
      ...
      applied_promotion: serializeAppliedPromotion(appliedPromotion, 'circuits'),
      ...
      <div className="payment-page">
        ...
        <div className="payment-success">
          <div className="payment-success__card">
            ...
            <h2 className="payment-success__title">{method === 'agency' ? 'Réservation confirmée !' : 'Paiement effectué !'}</h2>
            <p className="payment-success__desc">
              Merci <strong>{booking.prenom} {booking.nom}</strong> pour votre réservation du circuit <strong>{title}</strong>.{' '}
              {method === 'agency'
                ...
                : <>Votre paiement de <strong>{pricing.finalAmount.toLocaleString('fr-FR')} DT</strong> a été traité. Confirmation envoyée à <strong>{booking.email}</strong>.</>}
            </p>
            <div className="payment-success__actions">
              <button className="payment-success__btn" onClick={() => navigate('/circuits')}>← Voir d'autres circuits</button>
            </div>
          </div>
        </div>
        <Footer />
  ...
  return (
    <div className="payment-page">
      ...
      <div className="payment-hero" style={{ paddingTop: 170 }}>
        <div className="container">
          <div className="payment-breadcrumb">
            <button className="payment-breadcrumb__btn" onClick={() => navigate('/circuits')}>← Circuits</button>
          ...
          <div className="payment-steps">
            {[{ n: 1, label: 'Informations', state: 'done' }, { n: 2, label: 'Paiement', state: 'active' }, { n: 3, label: 'Confirmation', state: 'pending' }].map((step, i) => (
      ...
      <div className="payment-body">
        <div className="container">
          <div className="payment-layout">
            <div>
              <div className="payment-total-banner">
                <div>
                  ...
                  <div className="payment-total-banner__amount">{pricing.finalAmount.toLocaleString('fr-FR')} <span>DT</span></div>
                  <div className="payment-total-banner__sub">
                    {Number(price).toLocaleString('fr-FR')} DT × {personnes} participant{personnes > 1 ? 's' : ''}
                    {pricing.discountAmount > 0 && ` • réduction de ${pricing.discountAmount.toLocaleString('fr-FR')} DT`}
                  </div>
                </div>
              ...
              <div className="payment-card">
                ...
                <div className="pay-form-row" style={{ alignItems: 'flex-end' }}>
                  <div className="pay-field" style={{ flex: 1 }}>
                    ...
                    <input className="pay-input" type="text" value={promoCode} onChange={handlePromoChange} placeholder="Ex: ETE10" style={{ textTransform: 'uppercase' }} />
                  </div>
                ...
                <div className="payment-card">
                  ...
                  <form className="pay-form" onSubmit={handleOnlineSubmit}>
                    ...
                    <div className="payment-security-note">
                      ...
                      <div><div className="payment-security-note__title">Paiement 100% sécurisé</div><div className="payment-security-note__desc">Données chiffrées SSL 256-bit.</div></div>
                    </div>
                    <button type="submit" className="payment-submit-btn" disabled={loading}>
                      {loading ? '⏳ Traitement...' : `Payer ${pricing.finalAmount.toLocaleString('fr-FR')} DT →`}
                    </button>
                  </form>
                </div>
              )}
              ...
              {method === 'agency' && (
                <div className="payment-card payment-card--pink">
                  ...
                  {[
                    { icon: '📍', label: 'Adresse', value: 'Nouvelle Medina, Tunis' },
                  ...
                  <div className="agency-warning" style={{ marginTop: 16 }}>
                    ℹ️ Votre réservation sera retenue <strong>48h</strong>. Présentez-vous à l'agence avec votre confirmation.
            ...
            <aside className="payment-sidebar" style={{ top: 170 }}>
              <div className="payment-trip-card">
                ...
                <div className="payment-trip-card__body">
                  <div className="payment-trip-card__country">🗺️ Circuit Tunisie</div>
                  <div className="payment-trip-card__title">{title}</div>
                  <div className="payment-trip-card__meta">
                    {[{ icon: '🕐', text: duration }, { icon: '✈️', text: `Départ ${departure || 'Tunis'}` }, { icon: '👥', text: `${personnes} participant${personnes > 1 ? 's' : ''}` }, { icon: '🛏️', text: `Chambre ${booking.chambre}` }].map((item, i) => (
              ...
              <div className="payment-price-card">
                ...
                {[{ label: 'Prix / participant', value: `${Number(price).toLocaleString('fr-FR')} DT` }, { label: `× ${personnes} participant${personnes > 1 ? 's' : ''}`, value: `${totalPrix.toLocaleString('fr-FR')} DT` }, ...(pricing.discountAmount > 0 ? [{ label: 'Réduction promo', value: `- ${pricing.discountAmount.toLocaleString('fr-FR')} DT` }] : []), { label: 'Taxes & frais', value: 'Inclus' }].map((row, i) => (
                ...
                <div className="payment-price-total">
                  ...
                  <span className="payment-price-total__amount">{pricing.finalAmount.toLocaleString('fr-FR')} DT</span>
                </div>
              </div>
            </aside>
          </div>
