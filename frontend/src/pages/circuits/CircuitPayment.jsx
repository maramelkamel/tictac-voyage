    ...
    return (
      <div className="payment-page">
        ...
        <div style={{ textAlign: 'center', padding: '160px 24px' }}>
          ...
          <p style={{ fontSize: '18px', fontWeight: 700, color: '#0a2832', marginBottom: 16 }}>Reservation introuvable.</p>
          <button className="payment-fallback__btn" onClick={() => navigate('/circuits')}>
            Retour aux circuits
          </button>
        </div>
        <Footer />
        ...
        <div className="payment-success">
          <div className="payment-success__card">
            ...
            <p className="payment-success__desc">
              Merci <strong>{booking.prenom} {booking.nom}</strong> pour votre reservation de <strong>{title}</strong>.{' '}
              {method === 'agency'
                ...
                : <>Votre paiement de <strong>{pricing.finalAmount.toLocaleString('fr-FR')} DT</strong> a ete traite. Confirmation envoyee a <strong>{booking.email}</strong>.</>}
            </p>
            <div className="payment-success__actions">
              <button className="payment-success__btn" onClick={() => navigate('/circuits')}>
          ...
          <div className="payment-layout">
              ...
              <div className="payment-total-banner">
                <div>
                  ...
                  <div className="payment-total-banner__sub">
                    {Number(price).toLocaleString('fr-FR')} DT x {personnes} participant{personnes > 1 ? 's' : ''}
                    {pricing.discountAmount > 0 && ` • reduction de ${pricing.discountAmount.toLocaleString('fr-FR')} DT`}
                  </div>
                </div>
              ...
              <div className="payment-card">
                ...
                <div className="payment-method-row">
                  <button className={`payment-method-btn ${method === 'online' ? 'payment-method-btn--online' : ''}`} onClick={() => setMethod('online')}>
                    ...
                    <div className="payment-method-btn__desc">Carte bancaire, e-Dinar</div>
                  </button>
                  <button className={`payment-method-btn ${method === 'agency' ? 'payment-method-btn--agency' : ''}`} onClick={() => setMethod('agency')}>
                    ...
                    <div className="payment-method-btn__desc">Reglement en agence</div>
                  </button>
                </div>
              </div>
                ...
                <div className="payment-card">
                  ...
                  <form className="pay-form" onSubmit={handleOnlineSubmit}>
                    ...
                    <div className="pay-field">
                      ...
                      <input className="pay-input" name="cardName" type="text" placeholder="PRENOM NOM" required value={cardForm.cardName} onChange={handleCardChange} />
                    </div>
                    ...
                    <button type="submit" className="payment-submit-btn" disabled={loading}>
                      {loading ? 'Traitement en cours...' : `Payer ${pricing.finalAmount.toLocaleString('fr-FR')} DT`}
                    </button>
                  </form>
                </div>
              )}
            ...
            <aside className="payment-sidebar">
              <div className="payment-trip-card">
                ...
                <div className="payment-trip-card__body">
                  ...
                  <div className="payment-trip-card__meta">
                    {[duration, `Depart ${departure || 'Tunis'}`, `${personnes} voyageur${personnes > 1 ? 's' : ''}`, `Chambre ${booking.chambre}`].map((text, index) => (
              ...
              <div className="payment-price-card">
                ...
                {[
                  { label: 'Prix / participant', value: `${Number(price).toLocaleString('fr-FR')} DT` },
                  { label: `x ${personnes} participant${personnes > 1 ? 's' : ''}`, value: `${totalPrix.toLocaleString('fr-FR')} DT` },
                  ...(pricing.discountAmount > 0 ? [{ label: 'Reduction promo', value: `- ${pricing.discountAmount.toLocaleString('fr-FR')} DT` }] : []),
