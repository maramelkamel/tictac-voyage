// src/pages/hotels/HotelPayment.jsx
...
const API = 'http://localhost:5000/api/hotels/book';
...
const HotelPayment = () => {
  ...
  // ── Fallback ───────────────────────────────────────────────
  if (!state?.hotel || !state?.holder || !state?.hotelInfo) {
    return (
  ...
  const { hotel, holder, roomsPax, hotelInfo } = state;
  const { check_in, check_out, adults, children, rooms, total_amount, total_currency, rate_key } = hotelInfo;
  ...
  const currency    = total_currency || 'TND';
  ...
  // ── API call ────────────────────────────────────────────────
  const bookHotel = async (paymentMethod) => {
  ...
  const handleOnlineSubmit = async (e) => {
    ...
    try { await bookHotel('online'); setSubmitted(true); }
  ...
  const handleAgencyConfirm = async () => {
    setLoading(true); setApiError('');
    try { await bookHotel('agency'); setSubmitted(true); }
    catch (err) { setApiError(err.message); }
    finally { setLoading(false); }
  };
        ...
        <div className="payment-success">
          <div className="payment-success__card">
            ...
            <p className="payment-success__desc">
              Merci pour votre réservation à <strong>{hotel.name}</strong> du <strong>{fmtDate(check_in)}</strong> au <strong>{fmtDate(check_out)}</strong>.{' '}
              {method==='agency'
                ? <>Rendez-vous à notre agence pour finaliser le paiement. Un conseiller vous contactera sous 24h.</>
                : <>Votre paiement de <strong>{totalAmount.toLocaleString('fr-FR')} {currency}</strong> a été traité. Votre confirmation vous sera envoyée par email.</>
              }
            </p>
            <div className="payment-success__actions">
              ...
              <button className="payment-success__btn" onClick={() => navigate('/my-bookings')}
      ...
      <div className="payment-hero">
        <div className="container">
          <div className="payment-breadcrumb">
            <button className="payment-breadcrumb__btn" onClick={() => navigate('/hotels/search')}>← Hôtels</button>
          ...
          <div className="payment-steps">
            {[
              { n:1, label:'Voyageurs',  state:'done'    },
            ...
            <div>
              ...
              <div className="payment-total-banner">
                <div>
                  ...
                  <div className="payment-total-banner__sub">
                    {hotel.name} · {nights} nuit{nights>1?'s':''} · {adults+(children||0)} voyageur{(adults+(children||0))>1?'s':''}
                  </div>
                </div>
              ...
              <div className="payment-card">
                ...
                <div className="payment-method-row">
                  <button className={`payment-method-btn ${method==='online' ? 'payment-method-btn--online' : ''}`}
                    ...
                    <div className="payment-method-btn__desc">Carte bancaire, e-Dinar<br />Immédiat & sécurisé</div>
                    {method==='online' && <div className="payment-method-btn__badge payment-method-btn__badge--online">✓ Sélectionné</div>}
                  </button>
                  <button className={`payment-method-btn ${method==='agency' ? 'payment-method-btn--agency' : ''}`}
                ...
                <div className="payment-card">
                  ...
                  <div style={{ marginBottom:22 }}>
                    ...
                    <div className="card-types">
                      {[{key:'visa',label:'Visa',icon:'💳'},{key:'mastercard',label:'MC',icon:'🔴'},{key:'edinar',label:'e-Dinar',icon:'🇹🇳'},{key:'amex',label:'Amex',icon:'🟦'}].map(c => (
                  ...
                  <form className="pay-form" onSubmit={handleOnlineSubmit}>
                    <div className="pay-field">
                      ...
                      <div className="pay-input-wrapper">
                        <input className="pay-input pay-input--icon-right" name="cardNumber" type="text"
                    ...
                    <div className="pay-field">
                      ...
                      <input className="pay-input" name="cardName" type="text" placeholder="PRÉNOM NOM"
                        required value={cardForm.cardName} onChange={handleCardChange} style={{ textTransform:'uppercase' }} />
                    </div>
                    <div className="pay-form-row">
                      <div className="pay-field">
                        <label className="pay-label">Expiration *</label>
                        <input className="pay-input" name="expiry" type="text" placeholder="MM/AA"
                          required maxLength={5} value={cardForm.expiry} onChange={handleCardChange} />
                      </div>
                      <div className="pay-field">
                        <label className="pay-label">CVV *</label>
                        <div className="pay-input-wrapper">
                          <input className="pay-input pay-input--icon-right" name="cvv" type="password"
                            placeholder="•••" required maxLength={4} value={cardForm.cvv} onChange={handleCardChange} />
                          <span className="pay-input-info" title="Code au dos de la carte">ℹ️</span>
                        </div>
                      </div>
                    </div>
                    <div className="payment-security-note">
                      ...
                      <div>
                        ...
                        <div className="payment-security-note__desc">Données chiffrées SSL 256-bit.</div>
                      </div>
                    </div>
                    <button type="submit" className="payment-submit-btn" disabled={loading}>
                      {loading ? '⏳ Traitement…' : `Payer ${totalAmount.toLocaleString('fr-FR')} ${currency} →`}
                    </button>
                  </form>
                </div>
              )}
                ...
                <div className="payment-card payment-card--pink">
                  ...
                  <div style={{ borderRadius:14, overflow:'hidden', marginBottom:20, border:'1px solid var(--pay-border)' }}>
                    <iframe title="Tictac Voyages"
                      src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3194.5!2d10.1815!3d36.8065!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x0%3A0x0!2zMzbCsDQ4JzIzLjQiTiAxMMKwMTAnNTMuNCJF!5e0!3m2!1sfr!2stn!4v1600000000000"
                  ...
                  <div className="agency-warning" style={{ marginTop:16 }}>
                    ℹ️ Votre réservation sera retenue <strong>48h</strong>. Présentez-vous à l'agence avec votre email de confirmation.
                  </div>
            ...
            <aside className="payment-sidebar">
              <div className="payment-trip-card">
                ...
                <div className="payment-trip-card__body" style={{ padding:20 }}>
                  <div className="payment-trip-card__country">{hotel.city || hotel.destination_code}</div>
                  <div className="payment-trip-card__title">🏨 {hotel.name}</div>
                  <div className="payment-trip-card__meta">
                    {[
                      { icon:'📅', text:`Arrivée : ${fmtDate(check_in)}` },
                      { icon:'📅', text:`Départ : ${fmtDate(check_out)}` },
                      { icon:'🌙', text:`${nights} nuit${nights>1?'s':''}` },
                      { icon:'👥', text:`${adults+(children||0)} voyageur${(adults+(children||0))>1?'s':''}` },
                      { icon:'🛏️', text:`${rooms} chambre${rooms>1?'s':''}` },
                    ].map((item,i) => (
              ...
              <div className="payment-price-card">
                <div className="payment-price-card__title">Récapitulatif</div>
                {[
                  { label:`${nights} nuit${nights>1?'s':''} × ${rooms} ch.`, value:`${totalAmount.toLocaleString('fr-FR')} ${currency}` },
